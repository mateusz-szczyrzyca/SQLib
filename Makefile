# This Makefile is for the SQLib extension to perl.
#
...
TO_INST_PM = lib/SQLib.pm

PM_TO_BLIB = lib/SQLib.pm \
	blib/lib/SQLib.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/SQLib.pm $(INST_MAN3DIR)/SQLib.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               SQLib' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.05' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           A simple module to manage and store SQL queries in separate files.' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Mateusz Szczyrzyca <mateusz@szczyrzyca.pl>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>A simple module to manage and store SQL queries in separate files.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Mateusz Szczyrzyca &lt;mateusz@szczyrzyca.pl&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/SQLib.pm blib/lib/SQLib.pm 
